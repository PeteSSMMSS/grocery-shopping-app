...
class Product(Base):
    ...
    supermarket_id = Column(
        Integer, ForeignKey("supermarkets.id"), nullable=False, index=True
    )

    # Flexible pricing system
    price_type = Column(
        String(20), nullable=False, default="per_package"
    )  # per_package, per_kg, per_100g, per_liter
...
class ProductPrice(Base):
    ...
    )  # Price in cents (e.g., 299 for €2.99)
...
class PurchaseItem(Base):
    ...
    price_cents_at_purchase = Column(
        Integer, nullable=False
...
class MealIngredient(Base):
    ...
    )  # Quantity (flexible: grams, pieces, liters, etc.)
    quantity_unit = Column(
        String(10), nullable=False, default="g"
    )  # g, kg, stück, l, ml
